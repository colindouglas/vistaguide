...
```
# Summary
This dataset contains **`r listings$mls_no %>% unique() %>% length()` unique MLS listings**. The last update was performed on `r as_date(max(listings$datetime, na.rm = TRUE))`, and scraped **`r listings %>% filter(as_date(datetime) == as_date(max(datetime))) %>% pull(pid) %>% unique()`** different listings.
...
sqft_vs_time <- listings %>%    
  filter(status %in% c("For Sale", "Sold"),
         type %in% c("Single Family", "Condominium", "Duplex", "Triplex", "Land & Acreage", "Cottage & Recreational"),
...
sqft_plot <- sqft_vs_time %>% 
  ...
  geom_smooth(data = svt_fit, method = "loess", formula = y~x, aes(color = type), level = 0.01) +
