...
listings %>%
  ...
  xlab("Date Scraped") +
  scale_y_continuous(name = "Count") +
  geom_text(aes(y = count + max(count) * 0.075 * sign(count), label = count))
```
...
listings %>%
  ...
  geom_text(data = volume_changes, aes(y = enter + max(enter) * 0.075, label = enter)) +
  geom_text(data = volume_changes, aes(y = -(exit + max(exit) * 0.075), label = exit)) +
...
print_listing_table <- function(df) {
  df %>%
    mutate(street = ifelse(is.na(unit), street, paste0(unit, "-", street)),
           ...
           loc_bin = case_when(
             ...
             !is.na(postal_city) ~ postal_city,
             TRUE ~ city
           )) %>%
    select(`List Date` = list_date,
