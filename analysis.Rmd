...
This dataset contains **`r nrow(listings_all)` rows** describing **`r listings$mls_no %>% unique() %>% length()` unique listings**. The last update was performed **`r bin_datetime(last_dt)`**, and scraped **`r nrow(filter(listings, datetime > Sys.Date() - hours(18)))`** different listing pages.
...
volume_plot_daily <- listings_ts %>%
  ...
  scale_x_date(name = "List Date", date_labels = "%-m/%-d", date_breaks = "1 week") +
...
```{r Price per Square Foot Over Time, fig.width = 10, fig.height = 6}
...
sqft_plot <- svt_fit  %>% 
  ...
  facet_grid(type ~ loc_bin, scales = "free") +
