...
This dataset contains **`r nrow(listings_all)` rows** describing **`r listings_all$mls_no %>% unique() %>% length()` unique listings**. The last update was performed **`r bin_datetime(last_dt)`**, and scraped **`r nrow(filter(listings_all, datetime > Sys.Date() - hours(24)))`** different listing pages.
...
# Time on Market
Time between list date and the first time the listing shows up as "Sold". Showing HRM listings only
...
listings_ts %>%
  ...
  scale_x_date(name = "Week (Starting On)", date_labels = "%-m/%-d", date_breaks = "1 week") +
  scale_y_continuous(name = "Days to Sale") +
