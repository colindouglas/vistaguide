...
hrm_vs_other %>%
  ...
  geom_label(data = sqft_fits, aes(label = pretty),
             ...
             size = 3) +
  geom_point(aes(color = yday(ymd(list_date))), na.rm = TRUE, alpha = 0.3) +
...
sqft_vs_time <- listings %>%    
  filter(status %in% c("For Sale", "Sold"),
         type == "Single Family",
         ymd(list_date) > ymd("2020-04-01"))

price_sqft_fits <- sqft_vs_time %>%
...
listings_all %>%
  ...
  geom_bar(aes(fill = status), position = position_dodge2(preserve = "single"), width = 1) +
...
listings_all %>%
  ...
  facet_wrap(~ loc_bin, scales = "free_y", ncol = 1) +
